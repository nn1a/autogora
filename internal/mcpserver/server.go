...
func New(manager *boards.Manager, version string) (*mcp.Server, *Service) {
	...
	server := mcp.NewServer(&mcp.Implementation{Name: "autogora", Version: version}, &mcp.ServerOptions{Instructions: instructions})
...
func (s *Service) registerCore(server *mcp.Server) {
	addTool(server, "autogora_boards_list", "List Kanban boards", "List isolated boards with metadata, paths, and per-status task counts.", true, false, true, false, func(ctx context.Context, input boardsListInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
		return s.manager.List(ctx, input.IncludeArchived)
	})
	addTool(server, "autogora_boards_create", "Create Kanban board", "Create an isolated board with its own database, workspaces, attachments, and logs.", false, false, true, false, func(ctx context.Context, input boardCreateInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
	...
	addTool(server, "autogora_boards_update", "Update Kanban board", "Update board presentation metadata and its default project directory.", false, false, true, false, func(_ context.Context, input boardUpdateInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
	...
	addTool(server, "autogora_boards_switch", "Switch current Kanban board", "Persist the current board used when an explicit board is omitted.", false, false, true, false, func(_ context.Context, input boardSlugInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
		return s.manager.Switch(input.Slug)
	})
	addTool(server, "autogora_boards_remove", "Remove Kanban board", "Archive a named board by default, or permanently delete it when hard_delete is true.", false, true, false, false, func(_ context.Context, input boardRemoveInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
		return s.manager.Remove(input.Slug, input.HardDelete)
	})
	addTool(server, "autogora_create", "Create Kanban task", "Create a durable task assigned to Claude, Codex, Cline, Gemini, or a human.", false, false, false, false, func(ctx context.Context, input createInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
		if input.Runtime == "" {
	...
	addTool(server, "autogora_list", "List Kanban tasks", "List board tasks with optional status, assignee, runtime, and search filters.", true, false, true, false, func(ctx context.Context, input listInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
		if input.Limit == 0 {
	...
	addTool(server, "autogora_show", "Show Kanban task", "Read a task with dependencies, comments, run history, relationship graph, and bounded worker context.", true, false, true, false, func(ctx context.Context, input taskInput) (any, error) {
	...
	addTool(server, "autogora_context", "Build Kanban worker context", "Return the bounded task body, execution order, handoffs, attachments, prior attempts, and comments.", true, false, true, false, func(ctx context.Context, input taskInput) (any, error) {
		taskID, err := s.scopedTaskID(input.TaskID)
		if err != nil {
			return nil, err
		}
	...
	addTool(server, "autogora_graph", "Show Autogora relationship graph", "Show a bounded hierarchy and dependency DAG with execution phases.", true, false, true, false, func(ctx context.Context, input taskInput) (any, error) {
		taskID, err := s.scopedTaskID(input.TaskID)
		if err != nil {
			return nil, err
		}
	...
	addTool(server, "autogora_diagnostics", "Diagnose Kanban board", "Inspect task/run invariants, queue stalls, and active workers.", true, false, true, false, func(ctx context.Context, input boardInput) (any, error) {
		if err := s.requireAdmin(); err != nil {
			return nil, err
		}
		return usingStore(ctx, s, input.Board, func(opened *store.Store, board string) (any, error) { return opened.Diagnose(ctx, board) })
	})
	addTool(server, "autogora_events", "Read Kanban events", "Read the append-only board event stream by cursor, task, and kind.", true, false, true, false, func(ctx context.Context, input eventInput) (any, error) {
		if s.env("AUTOGORA_TASK_ID") != "" {
			var err error
	...
	addTool(server, "autogora_runs", "List Kanban runs", "Read full attempt history for one task.", true, false, true, false, func(ctx context.Context, input taskInput) (any, error) {
		taskID, err := s.scopedTaskID(input.TaskID)
		if err != nil {
			return nil, err
		}
	...
	addTool(server, "autogora_log", "Read Kanban worker log", "Read up to 1 MB from the tail of a task run log.", true, false, true, false, func(ctx context.Context, input logInput) (any, error) {
		taskID, err := s.scopedTaskID(input.TaskID)
		if err != nil {
			return nil, err
		}
...
func (s *Service) String() string { return fmt.Sprintf("Autogora MCP (%s)", s.manager.Current()) }
