...
func (s *Store) BuildWorkerContext(ctx context.Context, taskID string) (string, error) {
	...
	if omitted > 0 {
		lines = append(lines, fmt.Sprintf("- … %d additional related node(s) omitted from this bounded context; use kanban_graph from an orchestrator/admin session for the bounded topology view.", omitted))
	}
