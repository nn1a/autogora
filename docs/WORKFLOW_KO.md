...
아이디어를 `Triage`에 등록하고 작업을 구체화한 뒤 agent에게 맡기고, 검증 근거와 함께 `Done`으로 마무리한다. TUI, Web UI, CLI, MCP는 같은 SQLite 상태와 전이 규칙을 공유한다.
...
4. supervisor 또는 직접 실행한 dispatcher가 `Ready` 작업을 claim하고 agent를 실행한다.
5. agent가 테스트·검증 근거를 남기면 Autogora가 결과를 보존한 뒤 `Done`을 확정한다.
...
![첫 진입에서 전역 coding agent와 supervisor를 설정하는 실제 화면](images/workflow-00-agent-setup.png)

*전역 agent마다 model, provider, 역할, 동시 실행 상한과 fallback을 정하고 Web/TUI supervisor의 자동 시작 여부를 선택한다.*

`auto-start`를 저장하면 Web UI와 TUI가 열릴 때 in-process supervisor가 함께 시작되므로 별도 `dispatch --watch`가 필요 없다. Web UI의 `Agents`에서 현재 supervisor를 시작하거나 멈출 수도 있다. CLI로 같은 설정을 만들려면 다음 명령을 사용한다.

```bash
autogora agents detect
autogora agents detect --save
autogora agents defaults --worker codex --planner codex --judge codex
...
| `Ready` | 지금 claim할 수 있는 실행 대기 작업 | 담당자·비수동 runtime·의존성 조건 충족 | supervisor, dispatcher 또는 직접 claim |
| `Running` | worker가 원자적으로 claim한 작업 | supervisor, dispatcher 또는 `claim` | heartbeat 후 `Complete` 또는 `Block` |
...
Prerequisite가 끝나면 dependency edge의 handoff가 정확한 완료 run과 해당 change set을 고정한다. 이후 선행 task를 다시 실행하거나 보관해도 dependent가 소비할 결과는 바뀌지 않는다. 새 결과가 필요하면 dependent가 실행 중이 아닐 때 dependency를 unlink한 뒤 다시 link한다.

실행 중인 dependent의 prerequisite 목록은 고정된다. 실행이 끝날 때까지 dependency를 추가·제거하거나 prerequisite task를 삭제할 수 없다. 이 규칙은 실행 중 worker가 보던 입력이 바뀌는 일을 막는다.

후속 작업이 `worktree`를 사용하면 agent를 시작하기 전에 완료한 직접 prerequisite의 durable ref와 Git repository를 검증하고 고정된 head를 순서대로 merge한다. 모든 merge가 성공한 뒤 그 commit을 후속 run의 새 기준점으로 기록하므로 후속 change set에는 부모 파일이 반복되지 않고 후속 작업이 만든 변경만 남는다. 충돌이나 변조된 ref, 다른 repository의 change set을 발견하면 agent를 실행하지 않고 `Blocked`로 전환하며 `failureCount`는 늘리지 않는다. 공유 `dir`은 현재 HEAD가 필요한 parent head를 이미 포함한 경우에만 허용한다.
...
- 완료된 직접 prerequisite의 고정된 run, summary, metadata, change set 정보
...
*Planning과 Execution을 분리한 보드에서 흐름 전체를 가로 스크롤 없이 비교한다.*

### Review에 대한 중요한 차이

`Review`는 자동 품질 게이트가 아니라 사람이 카드를 잠시 보류하는 레인이다. dispatcher가 관리하는 worker의 `autogora_complete`는 완료 요청을 먼저 기록하며, 프로세스가 정상 종료되고 최종 artifact와 change set을 보존한 뒤에야 카드가 `Done`으로 이동한다.
...
Web UI에서는 `Triage` 컬럼 제목의 `+`를 누른다. 아직 담당자를 모르면 비워 두고, 요청 원문과 결정되지 않은 항목을 Description에 남긴다.
...
autogora decompose <task-id> \
  ...
  --finalizer-profile implementer:codex
```
...
전역 또는 board profile의 model/provider가 환경변수보다 우선한다. 인증 토큰은 board metadata나 전역 agent 설정에 저장하지 않고 각 coding agent의 로그인 또는 기존 환경변수를 사용한다.
...
Task 생성·편집 폼에서 Board profile을 선택하면 Assignee와 Runtime이 함께 채워지고 실제 적용될 model을 바로 확인할 수 있다. TUI의 Agent 섹션도 같은 profile 목록과 model 표시를 사용한다. Provider는 profile 설정과 run 이력에서 확인한다.

![Task 생성 폼에서 profile, runtime, model을 확인하는 실제 화면](images/workflow-01-task-routing.png)

*전역 설정과 board profile을 합친 실행 경로를 확인한 뒤 task를 저장한다.*
...
Web/TUI와 독립된 지속 프로세스가 필요할 때만 `dispatch --watch`를 사용한다.
...
CLI `claim`이나 MCP `autogora_claim`으로 dispatcher 없이 가져간 worktree run도 change set 보존 규칙을 지킨다. CLI `complete`와 MCP `autogora_complete`는 최종 tree를 `refs/autogora/runs/<run-id>`에 먼저 캡처한 뒤 동기적으로 `Done`을 확정한다. 캡처나 prerequisite history 검증에 실패하면 완료로 표시하지 않고 workspace를 보존한 채 `Blocked`로 전환한다.
...
# supervisor를 쓰지 않을 때만 직접 한 번 실행한다.
autogora dispatch --once --allow-writes --board product-web
```

Web/TUI supervisor가 실행 중이면 `promote` 뒤 두 번째 명령은 생략한다.
...
이 예제는 Git 저장소에서 실행하며 board의 `default-workdir`을 해당 저장소로 설정했다고 가정한다. 세 task 모두 격리된 worktree를 사용해 선행 변경을 자동으로 fan-in한다.
...
구현 run은 분석 change set의 고정된 head를, 리뷰 run은 구현 change set의 고정된 head를 자동으로 merge한 뒤 worker를 시작한다. 각 child change set의 기준 commit은 fan-in 뒤 head이므로 inherited parent 파일을 자신의 변경 파일로 중복 보고하지 않는다.
...
- 선택한 agent가 로그인되어 있고 quota를 사용할 수 있는가?
...
Block reason의 change set ID, prerequisite task ID와 충돌 파일을 확인한다. Autogora는 이 경우 worker를 시작하지 않고 실패 횟수도 늘리지 않는다. durable ref가 바뀌었거나 repository가 다르면 연결과 workspace 설정을 바로잡는다. 실제 merge 충돌이면 별도 통합 task에서 해결한 결과를 새 prerequisite로 연결한다. 실행 중인 dependent의 dependency는 바꿀 수 없으므로 먼저 run이 끝났는지 확인한다.

### 실패한 run의 부분 변경이 남음

Agent가 실패하거나 timeout·취소되기 전에 파일을 수정했거나 commit했다면 Autogora는 fallback과 자동 재시도를 멈추고 workspace를 보존한다. Block reason의 경로에서 diff와 commit을 검토한 뒤 필요한 변경은 별도 task나 branch로 옮긴다. 검토 없이 `Unblock`만 반복하면 서로 다른 agent의 변경이 충돌할 수 있다.
