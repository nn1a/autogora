...
```bash
# 읽기·분석 작업
taskcircuit dispatch --watch --max-workers 2 --board product-web

# 신뢰할 수 있는 코드 저장소에서 구현 작업도 허용
taskcircuit dispatch --watch --max-workers 2 --allow-writes --board product-web
```
...
```bash
taskcircuit boards create product-web \
  --name "Product Web" \
  --default-workdir "$PWD" \
  --switch
```
...
```bash
taskcircuit dispatch --once --allow-writes --board product-web
```
...
taskcircuit graph <task-id>

# hierarchy만 설정하거나 제거한다. 실행 순서는 바뀌지 않는다.
taskcircuit subtask-add <parent-task-id> <subtask-id> --position 0
taskcircuit subtask-rm <parent-task-id> <subtask-id>

# 실행 dependency를 추가한다. 첫 번째 ID가 prerequisite다.
taskcircuit link <prerequisite-id> <dependent-id>
taskcircuit unlink <prerequisite-id> <dependent-id>
```

MCP에서는 `taskcircuit_graph`, `taskcircuit_subtask_set`, `taskcircuit_subtask_remove`, `taskcircuit_link`, `taskcircuit_unlink`를 사용한다. 응답의 `parents`/`children` 필드는 각각 `prerequisites`/`dependents`를 뜻하고, hierarchy는 `parentTask`/`subtasks` 필드로 구분된다.
...
연결된 graph가 500개 node를 넘더라도 worker 시작을 실패시키지 않는다. `taskcircuit_graph`와 Web UI는 focus task, hierarchy root, 직접 관계를 우선해 최대 500개를 보여주고 `totalConnectedNodes`, `truncated`, `omittedNodeCount`로 생략 범위를 알린다. worker context는 이 중 필요한 50개 요약만 사용한다.
...
현재 일반 worker가 `taskcircuit_complete`를 호출하면 `Running`에서 바로 `Done`으로 간다. `Review`는 자동 품질 게이트가 아니라 사람이 카드를 잠시 보류하는 레인이다.
...
```bash
taskcircuit create "CSV 내보내기 요구사항 정리" \
  --triage \
...
```bash
taskcircuit specify <task-id> --planner-runtime codex
```

Cline을 planner로 사용할 수도 있다.

```bash
taskcircuit specify <task-id> --planner-runtime cline
```

외부 planner 없이 사람이 확정한 규격을 그대로 넣으려면 `--title`과 `--body`를 함께 사용한다.

```bash
taskcircuit specify <task-id> \
...
```bash
taskcircuit show <task-id>
```

### 5.3 Decompose: 분석·구현·검증 그래프 만들기

```bash
taskcircuit decompose <task-id> \
...
```bash
taskcircuit show <root-task-id>
taskcircuit list --sort status
```
...
```bash
taskcircuit dispatch --dry-run --max 3 --board product-web
```
...
taskcircuit terminate <task-id> --reason "관리자 수정 전 실행 종료"
```
...
```bash
taskcircuit comment <task-id> \
  "샌드박스 키를 비밀 저장소에 등록했고 웹훅 URL을 확정했습니다" \
  --author product-owner
taskcircuit unblock <task-id>
```
...
```bash
taskcircuit edit <task-id> --status review
```

승인하면 검증 요약과 함께 완료한다.

```bash
taskcircuit complete <task-id> \
...
```bash
taskcircuit create "검색 필터 초기화 버튼 구현" \
  --triage \
...
```bash
taskcircuit specify "$TASK_ID" --planner-runtime codex
taskcircuit show "$TASK_ID"
```
...
```bash
taskcircuit show "$TASK_ID"
taskcircuit runs "$TASK_ID"
taskcircuit log "$TASK_ID"
```
...
```bash
taskcircuit create "인증 토큰 갱신 흐름 분석" \
...
```bash
taskcircuit create "인증 흐름 문서 작성" \
...
```bash
taskcircuit create "인증 흐름 문서 정확성 검토" \
...
```bash
taskcircuit dispatch --watch --max-workers 1 --allow-writes --board product-web
```
...
```bash
taskcircuit create "API 타임아웃 원인 분석" \
...
```bash
taskcircuit create "API 타임아웃 처리 개선" \
...
```bash
taskcircuit create "API 타임아웃 변경 독립 리뷰" \
...
```bash
taskcircuit create "리뷰 지적 타임아웃 회귀 수정" \
...
```bash
FIX_ID=<수정-task-id>
taskcircuit link "$FIX_ID" "$REVIEW_ID"
taskcircuit unblock "$REVIEW_ID"
```
...
```bash
taskcircuit terminate <task-id> --reason "heartbeat 정지로 관리자 종료"
```
...
일반 worker는 최종 답변만 출력해서는 완료되지 않는다. `taskcircuit_complete` 또는 dispatcher가 제공한 scoped CLI `complete` 명령을 호출해야 한다. MCP가 비활성화된 Cline과 격리된 Gemini worker는 dispatcher prompt에 포함된 정확한 CLI lifecycle bridge를 사용한다.
...
taskcircuit stats
taskcircuit diagnostics
taskcircuit watch --since 0 --follow
```
