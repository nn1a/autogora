...
description: Decompose a goal into durable Kanban tasks, assignees, runtimes, workspaces, and acyclic dependencies. Use for planning or routing multi-step work across Claude, Codex, and Cline workers; do not use to implement claimed worker tasks.
---
...
5. Create each card with:
   ...
   - an explicit `assignee`, `runtime` (`claude`, `codex`, or `cline`), and workspace policy (`scratch`, `dir:<absolute-path>`, or `worktree`);
