...
1. Call `taskcircuit_list` to avoid duplicating existing work.
2. For a rough card already in `triage`, prefer `taskcircuit_specify` for one executable task or `taskcircuit_decompose` for an atomic routed graph. Decomposition records every generated node as a subtask of the triage root and separately records prerequisite/dependent execution edges. Supply the known profile roster and an explicit fallback; review the returned routes.
3. For a new swarm-shaped goal, use `taskcircuit_swarm` to create the completed blackboard, parallel workers, verifier, and synthesizer in one operation.
...
6. Use `taskcircuit_link` only for dependencies discovered after creation (`parent_id` is the prerequisite, `child_id` is the dependent). Use `taskcircuit_subtask_set` only for hierarchy ownership; hierarchy does not gate execution. Cycles are invalid in both relation types.
7. Call `taskcircuit_graph` and `taskcircuit_show` on the created cards. Check hierarchy membership, topological phases, root readiness, and that dependency-gated subtasks remain `todo` until every prerequisite handoff is satisfied.

If an administrative change must replace an active run, call `taskcircuit_run_terminate` first. Do not move, complete, archive, or delete a running task while its worker can still modify the workspace.
